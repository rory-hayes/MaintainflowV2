...
test("the browser release acceptance suite runs an isolated production build", () => {
  ...
  const fixture = readFileSync("tests/e2e/business-evals-test-fixture.ts", "utf8")
  const previewGate = readFileSync("src/lib/features/business-evals-preview.server.ts", "utf8")

  assert.match(config, /next build --webpack/)
  assert.match(config, /node \.next\/standalone\/server\.js/)
  assert.match(config, /cp -R \.next\/static \.next\/standalone\/\.next\/static/)
  assert.match(config, /cp -R public \.next\/standalone\/public/)
  ...
  assert.match(config, /MAINTAINFLOW_PLAYWRIGHT_PREVIEW_TOKEN/)
  assert.match(config, /process\.env\.MAINTAINFLOW_PLAYWRIGHT_PREVIEW_TOKEN = previewToken/)
  assert.match(config, /BUSINESS_EVALS_E2E_PREVIEW_TOKEN: previewToken/)
  assert.match(fixture, /x-maintainflow-e2e-preview-token/)
  assert.match(fixture, /requestUrl\.origin !== appOrigin/)
  assert.doesNotMatch(config, /extraHTTPHeaders/)
  assert.match(config, /NEXT_TELEMETRY_DISABLED: "1"/)
  assert.match(config, /BUSINESS_EVALS_LOCAL_E2E_ENV_CLEARED: "1"/)
  assert.match(config, /VERCEL: "0"/)
  assert.match(config, /VERCEL_ENV: "0"/)
  assert.match(config, /NEXT_PUBLIC_VERCEL_ENV: "0"/)
