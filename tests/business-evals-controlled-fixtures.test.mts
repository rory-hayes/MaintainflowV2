...
test("fixture submission cannot email outside the configured inbound domain or omit the synthetic marker", () => {
  ...
  assert.match(route, /fixtureSourceLimiter/)
  assert.match(route, /persistentFixtureRateLimitAllows/)
  assert.match(route, /rpc\/consume_business_eval_rate_limit/)
  assert.match(route, /p_window_seconds: 300/)
