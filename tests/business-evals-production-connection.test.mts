...
test("canary and launch have separate fail-closed checks and pushes", () => {
  ...
  assert.match(readiness, /Required environment is securely injected without \.env\.local/)
  assert.match(readiness, /\["dlx", "vercel@56\.3\.2", "--version"\]/)
  assert.match(readiness, /\["dlx", "vercel@56\.3\.2", "whoami"\]/)
